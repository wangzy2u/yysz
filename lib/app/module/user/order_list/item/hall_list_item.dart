...
class HallListItemView extends StatefulWidget {
  const HallListItemView({super.key});

  @override
  _HallListItemViewState createState() => _HallListItemViewState();
}

class _HallListItemViewState extends State {
  late Timer _timer;
  ...
  Widget build(BuildContext context) {
    return GetBuilder<OrderListLogic>(
        id: 'HallList',
        builder: (logic) {
          if (logic.hallList.isEmpty) {
            return const EmptyView();
          }
          return ListView.builder(
              padding: EdgeInsets.only(top: 0.w),
              itemCount: logic.hallList.length,
              itemBuilder: (context, index) {
                OrderListItemBean bean = logic.hallList[index];
  ...
  buildItem(OrderListItemBean item) {
    return Container(
            ...
            Row(
              ...
              children: [
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: GestureDetector(
                          onTap: () {
                            Clipboard.setData(
                                ClipboardData(text: '${item.orderNo}'));
                ...
                Container(
                  ...
                  child: Text(
                    getStateStr('${item.state}', item),
                    style: TextStyle(color: getStateColor('${item.state}')),
                  ),
                )
              ],
            ),
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                Container(
                                  padding:
                                      EdgeInsets.only(top: 18.w, left: 12.w),
                                  child: Text(
                                    '${item.collectionName}',
                                    style: TextStyle(
                                        fontSize: 14.sp, color: 1.skColor),
                                    maxLines: 1,
                                ...
                                Container(
                                  padding:
                                      EdgeInsets.only(top: 18.w, right: 12.w),
                                  child: Text(
                                    '¥${item.amount}',
                                    style: TextStyle(
      ...
      Get.toNamed(AppRoutes.orderDetail, arguments: {'id': item.id});
    });
  }
  ...
  String startCountdown(OrderListItemBean item) {
    ...
    time =
        '00:${remainingTime.inMinutes.toString().padLeft(2, '0')}:${(remainingTime.inSeconds % 60).toString().padLeft(2, '0')} 待付款';

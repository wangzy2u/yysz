...
class _TabStyle extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double _scale =
        (selected ? lerpDouble(_magnification, 1, animation.value) : lerpDouble(1, _magnification, animation.value))!;
...
double _indexChangeProgress(TabController controller) {
  ...
  // The logic's offset is changing because the user is dragging the
...
class _IndicatorPainter extends CustomPainter {
  ...
  Rect indicatorRect(Size tabBarSize, int tabIndex) {
    ...
    if (!(rect.size >= insets.collapsedSize)) {
      throw FlutterError('indicatorPadding insets should be less than Tab Size\n'
...
/// must be provided instead. The tab logic's [TabController.length] must
...
class ScaleTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  ///
  /// The [tabs] argument must not be null and its length must match the [logic]'s
  ...
  ///
  /// The length of this list must match the [logic]'s [TabController.length]
...
class _ScaleTabBarState extends State<ScaleTabBar> {
  ...
  Decoration get _indicator {
    ...
    // When that happens, automatic transitions of the skin will likely look
  ...
  // If the TabBar is rebuilt with a new tab logic, the caller should
  // dispose the old one. In that case the old logic's animation will be
    ...
    assert(() {
      if (newController == null) {
        throw FlutterError('No TabController for ${widget.runtimeType}.\n'
            ...
            'TabController using the "logic" property, or you must ensure that there '
            'is a DefaultTabController above the ${widget.runtimeType}.\n'
            'In this case, there was neither an explicit logic nor a default logic.');
      }
      return true;
    }());
  ...
  void _scrollToControllerValue() {
    ...
    if (value == index - 1.0)
      ...
      offset =
          (trailingPosition == null ? middlePosition : lerpDouble(middlePosition, trailingPosition, value - index))!;
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> wrappedTabs = List<Widget>.filled(widget.tabs.length, const Center());
    ...
    // If the logic was provided by DefaultTabController and we're part
    // of a Hero (typically the AppBar), then we will not be able to find the
    // logic during a Hero transition. See https://github.com/flutter/flutter/issues/213.
    if (_controller != null) {
      final int previousIndex = _controller!.previousIndex;

      if (_controller!.indexIsChanging) {
        // The user tapped on a tab, the tab logic's animation is running.

...
extension ExWidget on Widget {
  ...
  Widget frostedGlass({double ? sigmaX,double ? sigmaY,Color ? bgColor,double ? circular}){

    return ClipRRect(
      // 设置圆角
      borderRadius: BorderRadius.circular(circular ?? 0),
      child: BackdropFilter(
        filter: ImageFilter.blur(sigmaX: sigmaX ?? 10, sigmaY: sigmaY ?? 10),  // 模糊强度
        child: Container(
          decoration: BoxDecoration(
            color: bgColor ?? 2.skColor.withOpacity(0.2),  // 背景色+透明度
          ),
          child: this,
        ),
      ),
